...
CellSkewnessPlot_CellQuality <- ggplot(R1_Annotation_Accep_UnaccepAlble_SkewCImageValues, aes(x=factor(SkewC,levels=SkewCLevel), y=skewness))+ geom_violin(trim = T)+
...
CellSkewnessPlot_CellQuality <- CellSkewnessPlot_CellQuality +stat_compare_means(method = "t.test", label.y =0.9)+      # Add global p-value
  stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = "Typical")
#pdf(file = "./Image_Analysis_V3_CR_Methods_Review_20210523/Plots/GSE46980_CellSize_CellQulaity.pdf")
CellSkewnessPlot_CellQuality
#dev.off()
######### Status
### 20210523
## Plotting Cell Size
...
SkewCLevel <- factor(c("Typical","Skewed"))
CellSkewnessPlot_Status <- ggplot(R1_Annotation_Accep_UnaccepAlble_SkewCImageValues, aes(x=factor(SkewC,levels=SkewCLevel), y=skewness))+ geom_violin(trim = T)+
  geom_boxplot(width=0.1)+
...
theme(axis.ticks = element_line(colour = 'black',size = 1.2,linetype = 'solid'))+
  ...
  theme(panel.background = element_rect(fill = NA,colour = "black",size = 1.5,linetype = "solid"),
        panel.grid.major  = element_line(colour = "gray",size = 0.25))
CellSkewnessPlot_Status <- CellSkewnessPlot_Status +stat_compare_means(method = "t.test", label.y =0.9)+      # Add global p-value
  stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = "Typical")
#pdf(file = "./Image_Analysis_V3_CR_Methods_Review_20210523/Plots/GSE46980_CellSize_Status.pdf")
CellSkewnessPlot_Status
#dev.off()
